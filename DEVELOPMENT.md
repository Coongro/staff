...
- **Trigger name**: `trg_staff_{descriptive_name}`
- **Function name**: `trg_staff_{descriptive_name}_fn`

Where `staff` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_staff_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_staff_audit_log"
AFTER INSERT OR UPDATE ON "module_staff_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_staff_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_staff_*` triggers when the plugin
